#' @title Build fluid layout structure
#' @description Build structure for the fluid layout which uses a top navbar
#' @param navbar Navbar component
#' @param sidebar Not used
#' @param body Body content
#' @param footer Footer content
#' @keywords internal
layout_fluid <- function(navbar, sidebar, body, footer, theme = "light", color = NULL, show_theme_button = TRUE) {
  top_nav <- NULL
  ...
  if (!is.null(top_nav) && inherits(top_nav, "shiny.tag") && top_nav$name == "aside") {
    ...
    # Build horizontal layout header structure
    header_tag <- header(
